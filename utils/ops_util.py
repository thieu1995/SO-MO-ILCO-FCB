...
def find_closet_node(current_node, list_nodes):
    list_dist = array([current_node.dist(node) for node in list_nodes.values()])
    idx = argmin(list_dist)
    return list(list_nodes.values())[idx]


def is_instance_of_fog(fogs, current_id):
    for id_fog, fog in fogs.items():
        if current_id == id_fog:
            return fog
    return None


def get_cloud_node(clouds, current_id):
    for cloud_id, cloud in clouds.items():
        if current_id == cloud_id:
            return cloud

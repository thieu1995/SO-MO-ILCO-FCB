...
from utils.visual.scatter import visualize_3D, visualize_2D
...
class Root2(Root):
    """
    This class based on NSGA-II and NSGA-III models
    """
    ID_IDX = 0
    ID_POS = 1
    ID_FIT = 2
    ...
    def mutate(self, child, p_m):
        while True:
            ...
            child_pos = self.amend_position_random(child_pos)
            schedule = matrix_to_schedule(self.problem, child_pos.astype(int))
            if schedule.is_valid():
                fitness = self.Fit.fitness(schedule)
                idx = uuid4().hex
                break
    ...
    def crowding_distance(self, pop: dict, front: list):
        ...
        for i in range(self.n_objs):
            ...
            range_bound = float(max_value) - float(min_value) + self.EPSILON
            for k in range(1, len(front) - 1):
                value_1 = float(obj[i][int(sorted_list[i][k + 1])])
                value_2 = float(obj[i][sorted_list[i][k - 1]])
                distance[k] += (value_1 - value_2) / range_bound
        return distance

    # Function to calculate crowding distance + SDE
    def crowding_distance_SDE(self, pop: dict, front: list):
        obj = [zeros(len(pop)) for i in range(self.n_objs)]
        for idx, item in enumerate(pop.values()):
            for i in range(self.n_objs):
                obj[i][idx] = float(item[self.ID_FIT][i])
        ...
        for i in range(self.n_objs):
            ...
            for k in range(1, len(front) - 1):
                value_1 = float(obj[i][int(sorted_list[i][k + 1])])
                value_2 = float(obj[i][sorted_list[i][k - 1]])
                value_3 = float(obj[i][sorted_list[i][k]])
                value_1 = max(value_1, value_3)
                value_2 = max(value_2, value_3)
                distance[k] += (value_1 - value_2) / range_bound
        return distance
    ...
    def train(self):
        ...
        if Config.MODE == 'epoch':
            ...
            for epoch in range(self.epoch):
                ...
                break_loop = self.check_break_loop_multi(epoch + 1, fronts, pop, time_epoch_end, time_bound_start)
                if break_loop:
                    break

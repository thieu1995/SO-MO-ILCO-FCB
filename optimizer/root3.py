...
class Root3(Root):
    """
    This class based on MO-ALO model, basically using archive technique, probability technique to diverse the population
    """
    ID_IDX = 0
    ID_POS = 1
    ID_FIT = 2
    ...
    def train(self):
        ...
        if Config.MODE == 'epoch':
            ...
            training_info = self.adding_element_to_dict(training_info, ["Epoch", "FrontSize", "ArchiveSize", "Time"],
                                                        [epoch + 1, len(non_dominated_list), len(pop_archive), time_total])

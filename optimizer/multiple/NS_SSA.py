...
class BaseNS_SSA(Root2):
    ...
    def double_population(self, pop:dict, epoch:int):
        ...
        for i in range(0, n1):
            while True:
                if r2 < self.ST:
                    x_new = pop_new[list(pop_new.keys())[i]][self.ID_POS] * exp((epoch + 1) / self.epoch)
                else:
        ...
        for i in n2_list:
            while True:
                ...
                if i in fronts[0]:
                    ...
                    x_new = child[self.ID_POS] + uniform(-1, 1) * (abs(child[self.ID_POS] - current_best[self.ID_POS]) / (dist + self.EPSILON))
                x_new = self.amend_position_random(x_new)
                schedule = matrix_to_schedule(self.problem, x_new.astype(int))
                if schedule.is_valid():
                    fit = self.Fit.fitness(schedule)
    ...
    def evolve(self, pop=None, fe_mode=None, epoch=None, g_best=None):
        ...
        pop_temp = self.double_population(pop, epoch)

...
class I_AEO(Root):
    ...
    def evolve(self, pop=None, fe_mode=None, epoch=None, g_best=None):
        ...
        while True:
            child = (1 - a) * pop[-1][self.ID_POS] + a * uniform(self.lb, self.ub, self.problem["shape"])
            child = self.amend_position_random(child)
            schedule = matrix_to_schedule(self.problem, child.astype(int))
            if schedule.is_valid():
                fit = self.Fit.fitness(schedule)
        ...
        for i in range(2, self.pop_size):
            while True:
                ...
                ### Omnivore
                    ...
                    r2 = uniform()
                    child = pop[i][self.ID_POS] + c * (r2 * (pop[i][self.ID_POS] - pop[0][self.ID_POS])
            ...
            while True:
                ...
                if uniform() < 0.5:
                    ...
                    if uniform() < 0.5:
                        ...
                        child = beta * pop[i][self.ID_POS] + (1 - beta) * x_r
                else:
                    current_best[self.ID_POS] = current_best[self.ID_POS] + normal() * current_best[self.ID_POS]

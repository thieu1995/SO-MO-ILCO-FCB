...
class BaseGA(Root):
    """
        The original version of: Adaptive Genetic Algorithm (A-GA)
        Link:
            Cloud computing task scheduling algorithm based on improved genetic algorithm
            https://doi.org/10.1109/ITNEC.2019.8728996
    """
    ...
    def select(self, pop):
        ...
        while len(pop_new) < self.pop_size:
            ...
            #     mom_index = self.get_index_roulette_wheel_selection(fit_list)
            dad_index, mom_index = self.get_indexes_k_tournament_selection(fit_list, 6)
            dad = pop[dad_index]
            mom = pop[mom_index]

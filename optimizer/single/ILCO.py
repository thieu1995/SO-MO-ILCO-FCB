...
class ILCO(Root):
    ...
    def evolve(self, pop=None, fe_mode=None, epoch=None, g_best=None):
        ...
        for i in range(1, self.pop_size):
            while True:
                if i < K:
                    temp = (master - pop[i][self.ID_POS]) / (K - 1) - pop[i][self.ID_POS]
                    new_pop[i][self.ID_POS] = pop[i][self.ID_POS] + (temp + normal(0, 0.1, self.problem["shape"]))
                    ...
                    for j in range(n_l):
                        temp = new_pop[i][self.ID_POS] + normal(0, 0.1, self.problem["shape"])

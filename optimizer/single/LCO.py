        ...
        for i in range(0, self.pop_size):
            while True:
                ...
                if rand_number > 0.8:  # Update using Eq. 1, update from n best position
                    ...
                    # print(temp)
                    # print(array([pop[j][self.ID_FIT] for j in rd_index]))
                    ...
                    if random() < 0.8:
                        ...
                        temp = pop[i][self.ID_POS] + 0.5 * (uniform() * better_diff + uniform() * best_diff)
                    else:
                        temp = pop[i][self.ID_LOCAL_POS] + normal(0, 0.2, self.problem["shape"]) * a * (g_best[self.ID_POS] - pop[i][self.ID_POS])

...
from model.node.fog import Fog
from model.node.cloud import Cloud
from model.scheduler.task import Task
from model.node.peer import Peer
...
def create_cloud_node(name, location):
    ...
    cloud.eta = uniform(1e-8, 1e-6)         # latency - transmission
    cloud.lamda = uniform(5e-10, 5e-9)       # latency - processing
...
def create_fog_node(name, location):
    ...
    fog.eta = uniform(5e-10, 5e-9)       # latency - transmission
    fog.lamda = uniform(1e-8, 1e-7)     # latency - processing
